...
#if defined(NDEBUG)
# error "Genericoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x948b278c8d1b7b1eea9b3a682587d8d0e3cf883301843d02a57f79bd7bc9f6bf");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Genericoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Genericoin Signed Message:\n";
...
{
    // Genericoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Genericoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Genericoin: 2.5 days
static const int64 nTargetSpacing = 3.0 * 60; // Genericoin: 3.0 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Genericoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Genericoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool static LoadBlockIndexDB()
{
    ...
    printf("Load hashBestChain pointer to end of best chain\n");
    ...
    printf("set 'next' pointers in best chain");
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x81c14f6130ec40c92a6ea11de45d029a778f2343062b749ecfd43cad074c0bfe");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 21/Jan/2018 Bickering, Shutdown, Muddy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ac9529137db0b824bbedfbb9de490cd54b6aa6f292f901bb7b2a450efb2aae7d79e06ca1b4dce5c64567ebf1c5647cbafa8ab487863b8e2abd3a7dad1b82c4b7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516576535;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085065080;

        if (fTestNet)
        {
            block.nTime    = 1516576525;
            block.nNonce   = 386027703;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb8cb3de95964591a93e9b60ebf36dc9da854016d61be3953416787b81f0ef456"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Genericoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GenericoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GenericoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GenericoinMiner(CWallet *pwallet)
{
    printf("GenericoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("genericoin-miner");
    ...
    try { loop {
        ...
        printf("Running GenericoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GenericoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GenericoinMiner, pwallet));
}
